...
from training.datasets_art import data_gen, data_load, myDataset, preDataset
from training.plot_helpers import save_checkpoint, draw
...
def main():
    ...
    args = model_train_parameter(0, save, data, 32200)  # 32200
    start_epoch = 0
    start_subfold = 0
    # --------------1. Model Define--------------------------------
    V = 30
    ...
    subsets = [torch.utils.data.Subset(trainset, list(range(i * subset_size, (i + 1) * subset_size))) for i in range(10)]
    train_loader = [torch.utils.data.DataLoader(subset, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers) for subset in subsets]
    ...
    if args.resume:  # resume from last checkpoint
        if os.path.isfile(args.resumeLoc):
            ...
            start_subfold = checkpoint['subfold']
    ...
    if os.path.isfile(logFileLoc):
        ...
        logger.write("\n%s\t%s\t%s\t\t%s\t\t%s\t\t%s\t%s" % ('Epoch', 'Subfold', 'Loss(Tr)', 'Loss(val)', 'Loss(Ts)', 'Learning_rate', "Time"))
    logger.flush()

    # --------------7. Run Epoch-----------------------------------
    for epoch in range(start_epoch, args.max_epochs):
        print("Epoch No.: %d/%d" % (epoch, args.max_epochs))
        for subfold in range(start_subfold, 10):
            start_time = time.time()
            print("subfold No.: %d/%d" % (subfold, 10))
            # --------------a. training--------------------------------
            model.train()
            trainloss = run_epoch(data_load(train_loader[subfold], device=DEVICE), model, SimpleLossCompute(model.generator, criterion, model_opt))
            print("%d training finish %.4f" % (epoch, trainloss))
            # --------------b. validation & Testing--------------------
            model.eval()
            validloss = run_epoch(data_load(val_loader, device=DEVICE), model, SimpleLossCompute(model.generator, criterion, model_opt))
            testloss = run_epoch(data_load(test_loader, device=DEVICE), model, SimpleLossCompute(model.generator, criterion, model_opt))
            print("%d testing finish %.4f, %.4f" % (epoch, validloss, testloss))
            # --------------c. writing log-----------------------------
            logger.write("\n%d\t%d\t%.6f\t\t%.6f\t\t%.6f\t\t%.4f\t\t%.2f" % (epoch, subfold, trainloss, validloss, testloss, model_opt.rate(), time.time()-start_time))
            logger.flush()
            # --------------d. save checkpoint-----------------------------
            state = {'epoch': epoch,
                     'subfold': subfold + 1,
