...
def draw_psd(filename, data, window, snr_it, snr_id, sub1, sub2, order):
    ...
    freq = torch.fft.rfft(torch.tensor(data.cpu()), 1)
...
def numpy_SNR(origianl_waveform, target_waveform):
    # 位 dB
    origianl_waveform = np.array(origianl_waveform.cpu()).astype(np.float64)
    target_waveform = np.array(target_waveform.cpu()).astype(np.float64)
...
def numpy_SNR2(origianl_waveform, target_waveform):
    # 位 dB
    origianl_waveform = np.array(origianl_waveform.cpu().detach().numpy()).astype(np.float64)
    target_waveform = np.array(target_waveform.cpu().detach().numpy()).astype(np.float64)
    signal = np.sum(origianl_waveform ** 2)
    noise = np.sum((origianl_waveform - target_waveform) ** 2)
    snr = 10 * np.log10(signal / noise)
    if snr > 9999 or snr < -9999:
        snr = np.nan
    return snr
def SNR_cal2(input, target, decode):
    snr_i_t = []
    snr_i_d = []
    for batch in range(input.shape[0]):
        temp_i_t = []
        temp_i_d = []
        for channel in range(input.shape[1]):
            temp_i_t.append(numpy_SNR2(input[batch][channel], target[batch][channel]))
            temp_i_d.append(numpy_SNR2(input[batch][channel], decode[batch][channel]))
        snr_i_t.append(temp_i_t)
        snr_i_d.append(temp_i_d)
    ...
    snr_i_t = np.nanmean(np.nanmean(snr_i_t,axis=1))
    snr_i_d = np.nanmean(np.nanmean(snr_i_d,axis=1))

    return snr_i_t, snr_i_d



def partial_channel(signal1, signal2, partial_num):
